...
dfk = pd.DataFrame(get_rainfall(mysql,'54151-2',sdate,edate))
...
#dfk['MA10'] = (dfk['level'].rolling(window=10).mean())
#dfk = dfk.replace('NaN', 0,inplace=True)

# Add rainfall bins for the moving average
...
#   Create concatenated df for the heatmap
        ...
        df_join3.columns = [m1,m2,'Elfin Crossing','Kaputar Rainfall'] #,'Rainfall MA10','Rainfall Category'
        df_join3 = df_join3.apply(pd.to_numeric)
        df_join3 = df_join3.dropna()
...
# subplots https://plotly.com/python/subplots/
    ...
    fig = make_subplots(
                rows=3, cols=1,
                subplot_titles=("Groundwater", "Elfin Surfacewater", "Kaputar Rainfall", "KR (MA10)"))
...
#    fig.append_trace(go.Scatter(x=dfk1['read_date'], y=dfk1['MA10'],name='54151 (MA10)',opacity=1), row=4, col=1)
...
#    fig.update_yaxes(title_text="(mm)", row=4, col=1)
    fig.update_layout(title_text="Groundwater, Surfacewater Comparison",height=875,width=930) #height=875,width=930, textfont_color=
    ...
    fig2 = px.imshow(df_corr,title='Correlation Heatmap',height=500, width=480) #, labels= {""} ,height=500, width=480
