...
# meter_no_x_x read_date_x_x  level_x_x  meter_no_y_x read_date_y_x  level_y_x meter_no_x_y read_date_x_y  level_x_y meter_no_y_y read_date_y_y  level_y_y    
    ...
    for i in range(len(df)):
        ...
        row1 = [df.iloc[i,0],df.iloc[i,1],df.iloc[i,2],df.iloc[i,3],df.iloc[i,4]]
        row2 = [df.iloc[i,5],df.iloc[i,1],df.iloc[i,6]]
        row3 = [df.iloc[i,7],df.iloc[i,1],df.iloc[i,8]]
    ...
    df1 = pd.DataFrame(fields1,columns=['meter_no_x', 'read_date_x', 'level_x', 'meter_no_y',  'level_y'])
