...
all_training_data = 'data/waterdata/all_training_data.csv'

with open(all_training_data, 'w', newline='') as csvfile:
        writer = csv.writer(csvfile,dialect='excel')
        
        fields = ['read_date','bl_meter1','bl_ahd1','mean_temp1','bl_meter2','bl_ahd2','mean_temp2','bl_meter3','bl_ahd3','mean_temp3','bl_meter4','bl_ahd4','mean_temp4','sl_meter','sl_read1','rf_meter','rf_read1]'] 
        
        writer.writerow(fields)

        for i in range(len(df1)):
            
            row = [df1.iloc[i,0],df1.iloc[i,1], df1.iloc[i,2],df1.iloc[i,3],df1.iloc[i,4],df1.iloc[i,5],
